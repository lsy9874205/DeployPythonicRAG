...
LSY'S ANSWER ->Streaming allows us to send the reponse to the user in real-time. Doesn't need to wait for the entire response to be generated. Even though the steps in the pipeline are logically sequential, async ensures these steps don't block other tasks in the broader app. Multiple user queries can be processed in parallel.
...
LSY'S ANSWER: A User Session ensures each user's data is isolated from other users. Things like chat history, prefs, authentication (if used) details, etc. In distributed systems, global variables are specific to a specific server instance. A User Session is sotred in memory or in a session db, ensuring consistency across server instances.
